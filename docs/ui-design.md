...
- Knowledge Board is a full-width operational panel above Skills. Its first scan row shows service health, knowledge count, vector chunks, PostgreSQL capacity, average read latency, successful reads, and failed reads. The inventory supports search plus all/enabled/disabled filters, groups sorting by the governed package name, and shows that package beside each file title so repeated names remain distinguishable; selecting an item reveals chunks, token estimate, access tier, source, owner, and update time. Folder, green/red state, and trash actions reveal the exact source, enable/disable retrieval, or perform recoverable deletion.
- Knowledge Board v2.0 displays an `自动监测 · 200 秒` state in the header. It refreshes service and inventory status every 200 seconds without mutating the knowledge base; the icon-only refresh button remains the explicit full source-sync control.
